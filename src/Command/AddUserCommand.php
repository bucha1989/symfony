...
class AddUserCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption('isAdmin', '', InputArgument::OPTIONAL,
                'If set the user is created as an administrator', false);
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$isAdmin) {
            $isAdmin = $io->askQuestion(
                new Question('Is admin? (1 or 0)', 0)
            );
        }
        ...
        $stopwatchMessage = sprintf('New users id: %s / Elapsed time: %.2f ms / Consumed memory: %.2f mb',
            $user->getId(),
            $event->getDuration(),
